...
function validConnection(candidateHandles: Handle[]) {
    let [drageeRoad, targetRoad] = candidateHandles.map((handle) => getRoad(handle.parent, true) as RoadType);
    ...
    return (
        ...
        typeof drageeRoad.to !== "string" && typeof targetRoad.to !== "string"
    );
}
...
mouseState.subscribe((pos) => {
    ...
    if (!pos.down) {
        ...
        if (get(roads)[getRoadIndex(activeHandle.parent, true) as number].ghost) return;
